...
SESSION.headers.update({
    "User-Agent": "SummaryGameBot/1.0 (country history fetcher; educational project)"
})
...
def _strip_citation_html(html: str) -> str:
    ...
    # Remove <sup> tags (inline citation numbers like [1])
    html = re.sub(r"<sup[^>]*>.*?</sup>", "", html, flags=re.DOTALL)
    # Remove reference lists (<ol class="references">, <div class="reflist">)
    ...
    # Remove <table> tags (infoboxes, sidebars)
...
def fetch_country_history(country: str, wiki_url: str, wiki_country: str) -> str | None:
    """Try to fetch the History section for a country, with fallbacks."""
    titles_to_try = []

    if wiki_url:
        titles_to_try.append(title_from_url(wiki_url))
    if wiki_country and wiki_country not in titles_to_try:
        titles_to_try.append(wiki_country.strip())
    if country.strip() and country.strip() not in titles_to_try:
        titles_to_try.append(country.strip())
...
def write_country_histories():
    ...
    for index, row in enumerate(countries, start=1):
        ...
        output_name = f"{slugify_filename(wiki_country or country)}.txt"
        ...
        history_text = fetch_country_history(country, wiki_url, wiki_country)
        if not history_text:
