...
SESSION.headers.update({
    "User-Agent": "SummaryGameBot/1.0 (US state history fetcher; educational project)"
})
...
def fetch_state_history(state: str, wiki_url: str, wiki_state: str) -> str | None:
    """Try to fetch the History section for a US state, with fallbacks."""
    titles_to_try = []

    if wiki_url:
        titles_to_try.append(title_from_url(wiki_url))
    if wiki_state and wiki_state not in titles_to_try:
        titles_to_try.append(wiki_state.strip())
    if state.strip() and state.strip() not in titles_to_try:
        titles_to_try.append(state.strip())
...
def write_state_histories():
    ...
    for index, row in enumerate(states, start=1):
        ...
        output_name = f"{slugify_filename(wiki_state or state)}.txt"
        ...
        history_text = fetch_state_history(state, wiki_url, wiki_state)
        if not history_text:
